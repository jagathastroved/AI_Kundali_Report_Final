...
const getPlanetImage = (planetName: string) => {
  ...
  if (name.includes("moon") || name.includes("chandra"))
    return planetImages.moon;
  if (name.includes("mars") || name.includes("mangal") || name.includes("kuja"))
    return planetImages.mars;
  if (
    name.includes("mercury") ||
    name.includes("budh") ||
    name.includes("buddha")
  )
    return planetImages.buddha;
  if (
    name.includes("jupiter") ||
    name.includes("guru") ||
    name.includes("brihaspati")
  )
    return planetImages.guru;
  if (
    name.includes("venus") ||
    name.includes("shukra") ||
    name.includes("sukra")
  )
    return planetImages.sukra;
  if (
    name.includes("saturn") ||
    name.includes("shani") ||
    name.includes("sani")
  )
    return planetImages.sani;
...
const SouthChartCell = ({
  ...
  >
    {num && (
      <span className="absolute top-1 right-1.5 text-[9px] text-slate-400 font-medium">
        {num}
      </span>
    )}
    <div className="flex flex-col space-y-0.5 items-center justify-center w-full mt-1">
      {planets.map((p: string, i: number) => (
        <span
          key={i}
          className="text-[11px] sm:text-[12px] font-semibold page-text tracking-tight leading-none"
        >
          {p}
        </span>
      ))}
    ...
    {isAsc && (
      <span className="absolute bottom-1 right-1.5 text-[9px] font-bold text-indigo-500 uppercase tracking-widest">
        Lagna
      </span>
    )}
  </div>
      ...
      <div className="flex flex-col items-center justify-center w-full h-full text-center leading-none overflow-visible rounded-xl transition-colors duration-200 hover:bg-indigo-50/40">
        ...
        <div className="flex flex-col items-center justify-center mt-0.5 space-y-[1px]">
          {planets.map((p: string, i: number) => (
            <span
              key={i}
              className="text-[10px] font-semibold page-text tracking-tight leading-tight"
            >
              {p}
            </span>
          ))}
        ...
        {isAsc && (
          <span className="text-[8px] font-bold text-indigo-500 uppercase mt-1 tracking-widest">
            Lagna
          </span>
        )}
      </div>
...
export const LagnaChartPage: React.FC<{
  pageIdx: number;
  setPage: (idx: number) => void;
  isPdf?: boolean;
}> = ({ pageIdx, setPage, isPdf }) => {
  ...
  const baseStellium =
    findPageByType(data, "kundali_chart_stellium") ||
  ...
  const chartData =
    data?.pages?.page5_kundali_chart || data?.page5_kundali_chart;
  ...
  const northHousesCoords = [
    { x: 110, y: 30, w: 80, h: 90 },
    { x: 35, y: 5, w: 80, h: 60 },
    { x: 5, y: 35, w: 60, h: 80 },
    { x: 30, y: 110, w: 90, h: 80 },
    { x: 5, y: 185, w: 60, h: 80 },
    { x: 35, y: 235, w: 80, h: 60 },
    { x: 110, y: 180, w: 80, h: 90 },
    { x: 185, y: 235, w: 80, h: 60 },
    { x: 235, y: 185, w: 60, h: 80 },
    { x: 180, y: 110, w: 90, h: 80 },
    { x: 235, y: 35, w: 60, h: 80 },
    { x: 185, y: 5, w: 80, h: 60 },
  ];

  const getPlanetsForSign = (signName: string) => {
    return planetPositions
      .filter((p: any) => p.sign === signName && p.planet !== "Lagna")
      .map((p: any) => p.planet);
  };

  const isAsc = (signName: string) => {
    return planetPositions.some(
      (p: any) => p.sign === signName && p.planet === "Lagna",
    );
  };
  ...
  return (
    <div className="space-y-8 pb-6 relative">
      ...
      <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-50 to-white dark:from-slate-800/20 dark:to-transparent hover:dark:from-slate-900/40 hover:dark:to-slate-800/40 transition-all duration-300 rounded-3xl flex flex-col sm:flex-row justify-between items-center text-xs font-normal border border-default shadow-soft mx-1 gap-4">
        <div className="space-y-1 page-text text-center sm:text-left">
          ...
          <div className="text-[12px] text-muted flex flex-col sm:flex-row gap-1 sm:gap-3">
            <span>
              Date: {birthDetails?.day}/{birthDetails?.month}/
              {birthDetails?.year}
            </span>
            <span className="hidden sm:inline text-slate-300">|</span>
            <span>
              Time:{" "}
              {String((birthDetails?.hour || 0) % 12 || 12).padStart(2, "0")}:
              {String(birthDetails?.minute || 0).padStart(2, "0")}{" "}
              {(birthDetails?.hour || 0) >= 12 ? "PM" : "AM"}
            </span>
          </div>
        </div>
        ...
        {!isPdf && (
          <div className="flex relative p-1.5 rounded-full bg-slate-100/60 dark:bg-slate-800/60 backdrop-blur-md shadow-inner border border-slate-200/60 dark:border-slate-700/60 transition-colors">
            <button
              ...
              className={`relative z-10 px-6 py-2 rounded-full text-[11px] font-bold tracking-widest uppercase transition-all duration-300 overflow-hidden ${
                chartType === "north"
                  ? "text-white shadow-[0_4px_12px_rgba(99,102,241,0.4)]"
      ...
      {/* Interactive Graphic kundali Chart Wrapper */}
      <div
        className={`flex justify-center py-4 min-h-[340px] items-center ${isPdf ? "flex-row flex-wrap gap-8" : ""}`}
      >
        ...
        {(chartType === "south" || isPdf) && (
          <div className="grid grid-cols-4 grid-rows-4 gap-px bg-slate-800 dark:bg-slate-500 w-full aspect-square max-w-[340px] h-[340px] border border-slate-800 dark:border-slate-500 rounded-xl shadow-soft mx-auto overflow-hidden text-center font-sans relative">
            {/* Row 1 */}
            <SouthChartCell
              planets={getPlanetsForSign("Pisces")}
              isAsc={isAsc("Pisces")}
              num="12"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Pisces"), "12")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Aries")}
              isAsc={isAsc("Aries")}
              num="1"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Aries"), "1")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Taurus")}
              isAsc={isAsc("Taurus")}
              num="2"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Taurus"), "2")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Gemini")}
              isAsc={isAsc("Gemini")}
              num="3"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Gemini"), "3")
              }
              onMouseLeave={handleMouseLeave}
            />
            ...
            {/* Row 2 */}
            <SouthChartCell
              planets={getPlanetsForSign("Aquarius")}
              isAsc={isAsc("Aquarius")}
              num="11"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Aquarius"), "11")
              }
              onMouseLeave={handleMouseLeave}
            />
            <div className="col-span-2 row-span-2 card-bg relative flex flex-col items-center justify-center">
              <h3 className="text-xl font-bold page-text tracking-tight">
                Birth Chart
              </h3>
            ...
            <SouthChartCell
              planets={getPlanetsForSign("Cancer")}
              isAsc={isAsc("Cancer")}
              num="4"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Cancer"), "4")
              }
              onMouseLeave={handleMouseLeave}
            />
            ...
            {/* Row 3 */}
            <SouthChartCell
              planets={getPlanetsForSign("Capricorn")}
              isAsc={isAsc("Capricorn")}
              num="10"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Capricorn"), "10")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Leo")}
              isAsc={isAsc("Leo")}
              num="5"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Leo"), "5")
              }
              onMouseLeave={handleMouseLeave}
            />
            ...
            {/* Row 4 */}
            <SouthChartCell
              planets={getPlanetsForSign("Sagittarius")}
              isAsc={isAsc("Sagittarius")}
              num="9"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Sagittarius"), "9")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Scorpio")}
              isAsc={isAsc("Scorpio")}
              num="8"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Scorpio"), "8")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Libra")}
              isAsc={isAsc("Libra")}
              num="7"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Libra"), "7")
              }
              onMouseLeave={handleMouseLeave}
            />
            <SouthChartCell
              planets={getPlanetsForSign("Virgo")}
              isAsc={isAsc("Virgo")}
              num="6"
              onMouseMove={(e) =>
                handleMouseMove(e, getPlanetsForSign("Virgo"), "6")
              }
              onMouseLeave={handleMouseLeave}
            />
          </div>
            ...
            <div className="flex flex-col gap-2">
              {tooltip.planets.map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  <img
                    src={getPlanetImage(p)}
                    alt={p}
                    className="w-6 h-6 rounded-full border border-slate-200 dark:border-slate-700 shadow-soft"
                  />
