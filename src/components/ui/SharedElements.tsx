...
export const PieChartComponent: React.FC<{
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-center gap-8 bg-gradient-to-br from-slate-50 to-slate-100/50 dark:from-slate-800/40 dark:to-slate-900/40 hover:dark:from-slate-800/60 hover:dark:to-slate-900/60 border border-light p-6 sm:p-8 rounded-[2rem] shadow-soft hover:shadow-lg transition-all duration-300">
      <div
        className="relative w-48 h-48 sm:w-56 sm:h-56 drop-shadow-md flex items-center justify-center"
      ...
      >
        ...
        >
          {ratios.map((element, idx) => {
            ...
            return (
              <motion.path
                ...
                animate={{
                  opacity: 1,
                  scale: hoveredSlice?.name === element.name ? 1.05 : 1,
                }}
          ...
          <circle
            ...
            cy="80"
            r="32"
            className="fill-slate-50 dark:fill-slate-900/90 pointer-events-none"
          />
        </svg>
          ...
          {(() => {
            const displaySlice =
              ...
              (() => {
                ...
                const max = ratios.reduce((prev, current) =>
                  prev.percentage > current.percentage ? prev : current,
                );
                return {
                  name: max.name,
                  percentage: max.percentage,
                  color: textColors[max.name],
                };
              })();
            ...
            return (
              ...
              >
                <span
                  className={`block text-[10px] sm:text-xs font-bold uppercase tracking-wider ${displaySlice.color}`}
                >
                  {displaySlice.name}
                </span>
                <span
                  className={`block text-xl sm:text-2xl font-black ${displaySlice.color}`}
                >
      ...
      <div className="space-y-2.5 flex-1 min-w-[140px] bg-white/50 dark:bg-slate-800/30 p-4 rounded-2xl border border-light">
        {ratios.map((element, idx) => {
          ...
          return (
            <div
              key={idx}
              className="flex items-center text-[13px] font-medium page-text group"
            >
              <span
                className="w-3.5 h-3.5 rounded-full mr-3 shadow-sm transition-transform group-hover:scale-110"
                style={{ backgroundColor: color }}
              />
              <span className="min-w-[55px] tracking-wide">
                {element.name}:
              </span>
              <span
                className={`ml-2 font-bold ${textColors[element.name] || "text-slate-500"}`}
              >
                {element.percentage}%
              </span>
            </div>
...
export const renderPromoBox = (
  onNext: () => void,
  variant: "combo" | "remedies" | "element" | "planetary" = "combo",
) => {
  return (
      ...
      <div className="flex-1 space-y-4 sm:space-y-5 relative z-10 flex flex-col justify-center text-center sm:text-left">
        <div>
          ...
          <h4 className="text-xl sm:text-2xl font-bold text-white leading-tight tracking-tight drop-shadow-sm px-1 sm:px-0">
            {variant === "combo"
              ? "Unlock Personalized Full Kundali Report"
              : variant === "element"
                ? "Unlock Your Complete Astrological Destiny"
                : variant === "planetary"
                  ? "Unlock Your Premium Kundali Report"
